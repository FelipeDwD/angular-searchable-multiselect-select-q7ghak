...
export class SelectCustomTriggerExample {
  ...
  ngOnInit() {    
    ...
    if(this.data.length > 1)
      this.data.unshift({ id: 0, name: this.config.selectAllText.toUpperCase() })      
  }
 ...
 selectionChange(event) {     
  ...
  if (indexList == -1)
      ...
      this.selectedItems.splice(indexList, 1);


  if (this.selectedItems.filter(x => x.id != 0).length == this.data.length - 1) {
    this.selectAll = true;
    ...
    if(this.skillSel.options.find(x => x.value.id == 0) != null)
      ...
      if(indexSelectAll != -1)
        this.selectedItems.splice(indexSelectAll, 1)
